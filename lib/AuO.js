...
const AuO = function (SAVE_URL = null) {
    ...
    const animateAudioDisplay = function () {
        const viewWidth = audioUI.element().clientWidth - 2 * visualizerBuffer;
        ...
        if (!state.drawing && state.dataUpdated) {
            ...
            audioVisualizer.style("width", (width + 2 * visualizerBuffer).toString() + "px");
    ...
    const animateAudioTicker = function () {
        const currentTime = state.audioPlaybackCurrentTime();
        const timeFraction = (0 === state.elapsedTime ? 0 : currentTime / state.elapsedTime);
        const canvasWidth = audioDisplay.get("width");
        const tickerWidth = audioTicker.element().offsetWidth;
        const tickerLocation = Math.max(0, Math.min(1, timeFraction)) * canvasWidth +
            visualizerBuffer - 0.5 * tickerWidth;
    ...
    const beginAudioPlayback = function (start = 0, end = Infinity) {
        ...
        playbackSource.start(playbackStartTime, start, end);
        state.audioPlaybackCurrentTime = function () {
            return audioContext.currentTime - playbackStartTime + start;
        };
    };
    ...
    const beginAudioSave = function (onready, start = 0, end = Infinity) {
        ...
        // TODO: same note as in audio recording about using AudioWorkerNode once it is available.
        const saveBuffer = audioContext.createBuffer(1, dataLength, sampleRate);

        // Populate the buffer.
        var sample = 0;
        for (const series of state.data) {
            for (const value of series) {
                saveBuffer.getChannelData(0)[sample] = value;
                sample++;
            }
        }
        ...
        saveRecorder.start();
        saveSource.start(audioContext.currentTime, start, end);
    };
    ...
    buttonPlay.listen("click", function (event) {
        ...
        const currentTime = state.audioPlaybackCurrentTime();
        const totalTime = state.elapsedTime;
        const startTime = (currentTime > totalTime || timeEqualsA(currentTime, totalTime) ?
            0 : currentTime);
        state.audioTickerSync = true;
        beginAudioPlayback(startTime);
    });
...
disabled. Please instantiate AuO with an upload URL if you wish to enable online recording.");
    ...
    audioTicker.listen("dragstart", function (event) {
        ...
        state.audioOnDrag = function (x) {
            if (0 !== state.elapsedTime) {
                const canvasWidth = audioDisplay.get("width");
                const tickerWidth = audioTicker.element().offsetWidth;
                x = Math.max(0, Math.min(canvasWidth, x - 50));
                state.audioPlaybackCurrentTime = function () {
                    return state.elapsedTime * x / canvasWidth;
                };
                animateAudioTicker();
            }
        };
    ...
    // Attach drop handlers for the ticker and the container.
    for (node of [audioTicker, audioVisualizer, container]) {
    ...
    audioTicker.class("auo-audio-ticker").set("draggable", "true");
...
(function () {
    ...
    Sheet.rule(`.auo-audio-display {
        ...
        margin: 0px ` + visualizerBuffer.toString() + `px;
        height: 100px;
        width: calc(100% - ` + (2 * visualizerBuffer).toString() + `px);
    }`);
