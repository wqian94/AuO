...
const AuO = function (SAVE_URL = null) {
    ...
    const animateAudioTicker = function () {
        ...
        const tickerLocation = Math.max(0, Math.min(1, timeFraction)) * canvasWidth +
            VISUALIZER_BUFFER - 0.5 * tickerWidth;
    ...
    const endAudioRecording = function () {
        ...
        state.audioBuffer = audioContext.createBuffer(1, dataLength, sampleRate);

        // Populate the buffer.
        var sample = 0;
        for (const series of state.data) {
            for (const value of series) {
                state.audioBuffer.getChannelData(0)[sample] = value;
                sample++;
            }
        }
    ...
    const processAudioRecording = function (event) {
        ...
        if (state.recording) {
            ...
            if (state.endRecording) {
                setTimeout(endAudioRecording, 0);
                state.endRecording = false;
            }
        }
