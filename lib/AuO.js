...
const AuO = function () {
    ...
    const animateAudioDisplay = function () {
        ...
        const index = state.dataIndicesProcessed;

        // We have new data to draw.
        if (state.running && index < state.data.length) {
            state.dataIndicesProcessed++;
            const sample = state.dataSamplesProcessed;
            state.dataSamplesProcessed += state.data[index].length;
            audioDisplay.set("width", sampleToX(state.dataSamplesProcessed));
            ...
            // Set up coordinate iteration.
            var i = sample;
            if (0 < index) {
                canvas.moveTo(sampleToX(i - 1),
                    valueToY(state.data[index - 1][state.data[index - 1].length - 1]));
            }

            // Iterate along the data and plot.
            for (const value of state.data[index]) {
                const x = sampleToX(i);
