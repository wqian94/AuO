...
const AuO = function (SAVE_URL = null, SAVE_CALLBACK = null) {
    ...
    buttonTag.listen("click", function (event) {
        ...
        const label = prompt("Please enter a label for the tag at " + time +
            "s. Leave the field blank to use the time as the label.", time + "s");
        state.tags[parseFloat(time)] = (!isNil(label) && 0 < label.length ? label : time + "s");
        state.tagsSynced = false;
    });
