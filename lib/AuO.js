...
const AuO = function (SAVE_URL = null, SAVE_CALLBACK = null) {
     ...
     */
    const hotkeyListener = function (event) {
    ...
    buttonTag.listen("click", function (event) {
        const time = state.audioPlaybackCurrentTime().toFixed(3);
        state.tags.add(time);
        state.tagsSynced = false;
    });
    ...
    container.listen("keydown", hotkeyListener);
    ...
    loadFileLabel.class("auo-load-file-label");
    loadFile.class("auo-load-file").set("accept", getSupportedMediaTypes().join(","))
        .set("placeholder", "Enter URL here");
