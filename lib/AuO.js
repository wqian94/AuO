...
const AuO = function (SAVE_URL = null, SAVE_CALLBACK = null) {
    ...
    this.getTags = function () {
        const tags = [];
        state.tags.forEach(function (time) {
            tags.push(parseInt(1000 * parseFloat(time)));
        });
        return tags;
    };
    ...
    const tagsUpdateUI = function () {
        ...
        while (tags.length > tagIndex && listChildren.length > eleIndex) {
            const tagTime = tags[tagIndex];
            const eleTime = parseFloat(listChildren[eleIndex].textContent);

            // Using listChildren lets us avoid dealing with messy indices.
            if (eleTime < tagTime) {
                list.removeChild(listChildren[eleIndex]);
                eleIndex++;
            } else if (tagTime < eleTime) {
                list.insertBefore(tagCreateLabel(tagTime).element(), listChildren[eleIndex]);
                tagIndex++;
            } else {
                eleIndex++;
                tagIndex++;
        ...
        while (tags.length > tagIndex) {
            tagList.append(tagCreateLabel(tags[tagIndex]));
            tagIndex++;
        }
    };
     ...
     */
    const tagCreateLabel = function (time) {
        return new FunctionalElement("label").set("innerHTML", time.toString() + "s")
            .set("title", "Left-click to jump to tagged time, shift-click to delete tag.")
    ...
    const tagClickHandler = function (event) {
        const time = parseFloat(this.textContent).toFixed(3);
        if (event.shiftKey) {  // Delete.
            state.tags.delete(time);
            state.tagsSynced = false;
    ...
    const hotkeyHandler = function (event) {
        ...
        switch (event.code) {
            case "ArrowLeft":  // Left arrow key.
                ...
                } else {  // Neither Ctrl nor Shift modifiers.
                    if (!(state.playing || state.recording)) {
                        const currentTime = Math.max(state.trimStart, Math.min(state.elapsedTime - state.trimEnd,
                            state.audioPlaybackCurrentTime()));
            ...
            case "BracketLeft":
                if (!(state.playing || state.recording)) {
                    if (event.shiftKey) {  // Shift key.
                        ...
                        state.trimStart = Math.max(minTime, Math.min(maxTime, state.trimStart - dt));
            ...
            case "KeyF":  // F key.
                const x = convertUnits(state.audioPlaybackCurrentTime(), state.elapsedTime,
                    audioVisualizer.element().clientWidth) - 0.5 * audioUI.element().clientWidth;
