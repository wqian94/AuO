...
const AuO = function (SAVE_URL = null) {
    ...
    const animateAudioTicker = function () {
        ...
        const tickerOffset = convertUnits(currentTime, state.elapsedTime, canvasWidth);
        const tickerLocation = Math.max(0, Math.min(canvasWidth, tickerOffset)) +
    ...
    const beginAudioPlayback = function (start = 0, end = Infinity) {
        ...
        const playbackStartTime = audioContext.currentTime + 0.1;
