...
const AuO = function () {
    ...
    const runtimeAtConstruction = function () {
        ...
        state.viewWidth = audioDisplay.element().clientWidth;
        state.viewHeight = audioDisplay.element().clientHeight;
    ...
    const runtimeAtInitiation = function () {
        ...
        audioDisplay.set("width", audioDisplay.element().clientWidth);
    ...
    const animateAudioDisplay = function () {
        ...
        if (!state.drawing && state.dataUpdated) {
            ...
            const sampleToX = function (sample) {
                return sample * zoomFactor() * canvasWidth / totalSamples;
            };
            ...
            // Cumulative number of series and number of samples.
            for (var seriesIndex = state.dataIndicesProcessed; seriesIndex < index; seriesIndex++) {
                state.dataSamplesProcessed += state.data[seriesIndex].length;
            }
            state.dataIndicesProcessed = index;
            ...
            const samplesPerInterval = Math.max(1, Math.round(totalSamples / canvasWidth / 100));

            // Iterate along the data and plot.
            var sample = 0, series = 0;
            for (series = 0; series < index; series++) {
    ...
    const beginAudioRecording = function (stream) {
        ...
        const bufferNode = audioContext.createScriptProcessor(0, 1, 1);
        bufferNode.onaudioprocess = processAudioRecording;

        streamSource.connect(bufferNode);
